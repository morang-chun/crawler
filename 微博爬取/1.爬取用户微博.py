...
def get_one_page(url):#获取用户主页上的所有微博的信息
    ...
    'Cookie' : 'WEIBOCN_FROM=1110006030; loginScene=102003; \n'
               'SUB=_2A25OeR-SDeRhGeBK71oX8SbOwj2IHXVthaHarDV6PUJbkdANLRHbkW1NR6PhDV-zDyLHuRMDAxQSh59HOeuxzs-S; \n'
               '_T_WM=75189904294;\n'
               ' XSRF-TOKEN=8547be; mweibo_short_token=fddb9fb857;\n'
               ' MLOGIN=1; M_WEIBOCN_PARAMS=lfid%3D102803%26luicode%3D20000174%26uicode%3D20000174',
    'DNT' : '1',
...
def parse_one_page(html,uid):#解析文本并存储有用信息
    ...
    result2 = str(items2)
    #将所有微博的id及其页数写入本地文本
    with open('E:\PYTHON临时\微博爬取微博id和页数\人民日报\{}.txt'.format(uid),'a',encoding='utf-8') as fp:#文件路径自替换，后同
        fp.write(result1)
        fp.write(result2)
...
def main():
    uid='2803301701'#用户的uid，可在主页网址栏中找到
    PageNum=3#用户微博页数
