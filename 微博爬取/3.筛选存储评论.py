...
def make_book(uid):
    ...
    #为每条微博添加表单
    while os.path.exists('E:\PYTHON临时\爬取的评论\人民日报\微博{}of{}.txt'.format(k+1,uid)):
        sheet = book.add_sheet('微博'+str(k+1),cell_overwrite_ok=True)
        content = open('E:\PYTHON临时\爬取的评论\人民日报\微博{}of{}.txt'.format(k+1,uid),'r',encoding='utf-8').read()#读取文件二
        #初步提取评论中的文字
